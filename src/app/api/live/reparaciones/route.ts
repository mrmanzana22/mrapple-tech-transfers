...
import { isLiveSnapshotEnabled, readLiveRepairsByTecnico, upsertLiveRepairs, refreshTeamSummary } from "@/lib/live-snapshot";
...
async function fetchRepairsFromN8n(tecnicoQuery: string): Promise<unknown[]> {
  const url = `${N8N_BASE}/tech-reparaciones?tecnico=${encodeURIComponent(tecnicoQuery)}`;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `live:reparaciones:${tecnicoQuery}`;
    ...
    if (isLiveSnapshotEnabled()) {
      try {
        ...
        console.error("live repairs read error:", error);
      }
    }
    ...
    console.error("GET /api/live/reparaciones error:", error);
    const res = NextResponse.json(
